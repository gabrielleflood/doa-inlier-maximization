...
bp = '/Users/magnuso/Jobbmapp/projekt/mylocal_audiovisual_drone_detection/data/ljungbyhed_audio_dataset/';
...
set(ll,'LineWidth',2)
%ll = plot(angle_est_time(id),th_max(id),'--');
%set(ll,'LineWidth',2)

%ll = plot(angle_est_time(id(ok)),th_max(id(ok))*sf,'.');
...
set(ll,'LineWidth',2)
%ll = plot(angle_est_time(id),phi_max(id),'--');
%set(ll,'LineWidth',2)

%ll = plot(angle_est_time(id(ok)),phi_max(id(ok))*sf,'.');
